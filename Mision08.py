...
def sumarAcumulado(lista): # recibe una lista de números enteros y regresa una nueva con la suma
    ...
    for k in range(len(lista)):
        ...
        Suma.append(i) # se le agrega a la lista vacía el valor de "i"
    return Suma
...
def sonAnagramas(stringUno, stringDos): # verifica si un par de palabras son anagramas
    ...
    listaUno.sort() # ordena los datos de la lista y  modifica la lista
    listaDos.sort()
...
def hayDuplicados(lista): # recibe una lista y recorre cada valor de ella, si hay duplicado regresa true si no regresa false
    for numero in lista:
        duplicados = lista.count(numero) # regresa el número de veces que está el objeto por dentro de la lista
        if duplicados > 1: # si es mayor a uno la función regresa "True" de que, si hay duplicados en la lista
         return True
    else:
        return False # si no,  regresa "false" de que no hay duplicados en la lista


def borrarDuplicados(lista): # Genera una nueva lista que no tenga duplicados
    for k in range(len(lista) -1,-1,-1):
        duplicados = lista.count(lista[k]) # regresa el número de veces que está el objeto por dentro de la lista
        if duplicados > 1: # si es mayor a uno se elimina de la lista original y crea una lista nueva sin los datos duplicados
         lista.remove(lista[k]) # elimina el objeto x de la lista
